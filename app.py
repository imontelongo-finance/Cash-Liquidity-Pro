...
c1.metric("Projected Liquidity", f"€{curr_bal:,.0f}")
c2.metric("Net Weekly Burn/Gen", f"€{df_cf['Net Cash Flow'].mean():,.0f}")
c3.metric("Waste Savings", f"€{(f_ops['Waste_Loss'].sum() * eff_gain):,.0f}")
c4.metric("WC Cash Unlocked", f"€{cash_unlocked:,.0f}")

# Main Layout
tab1, tab2 = st.tabs(["📈 Liquidity Projection", "📊 Operational Drill-Down"])

with tab1:
    st.plotly_chart(px.line(df_cf, x='Week', y='Ending Balance', title="13-Week Cash Balance Forecast (Direct Method)"), use_container_width=True)
...
with tab2:
    ...
    with col_a:
        st.plotly_chart(px.bar(f_ops.groupby('Hub_Location')['CM2_Net_Cash'].sum().reset_index(), 
                               x='Hub_Location', y='CM2_Net_Cash', title="Margin Contribution by District"), use_container_width=True)
    with col_b:
        st.plotly_chart(px.pie(f_ap, values='Amount_EUR', names='Category', title="Expense Distribution by Category"), use_container_width=True)